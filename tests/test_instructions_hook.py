...
_PLUGIN_COMMANDS_DIR = Path(__file__).parent.parent / "extensions" / "gh_management" / "github_planner" / "commands"
...
def test_plugin_workflow_file_exists(filename):
    assert (_PLUGIN_COMMANDS_DIR / filename).exists(), f"Missing: extensions/gh_management/github_planner/commands/{filename}"
...
def test_verify_auth_failure_includes_guidance(tmp_path):
    with patch("extensions.gh_management.github_planner.get_workspace_root", return_value=tmp_path), \
         patch("extensions.gh_management.github_planner.verify_gh_cli_auth", return_value=(False, "Not logged in")):
        s = create_server()
