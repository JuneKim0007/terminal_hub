...
def test_resolve_token_cache_hit():
    ...
    from extensions.github_planner.auth import resolve_token, invalidate_token_cache
    invalidate_token_cache()

    with patch("extensions.github_planner.auth.subprocess.run") as mock_run:
...
def test_commands_load_failure():
    ...
    import extensions.github_planner.commands as cmd_mod
...
def test_read_doc_file_unknown_key(workspace):
    ...
    from extensions.github_planner.storage import read_doc_file
...
def test_read_doc_file_oserror(workspace):
    ...
    with patch("extensions.github_planner.storage.Path.read_text", side_effect=OSError("perm denied")):
