...
def test_atomic_write_cleans_up_tmp_on_error(tmp_path):
    ...
    with patch("extensions.gh_management.github_planner.storage.os.replace", side_effect=bad_replace):
...
def test_atomic_write_unlink_oserror_is_suppressed_and_exception_still_raises(tmp_path):
    ...
    with patch("extensions.gh_management.github_planner.storage.os.replace", side_effect=bad_replace), \
         patch("extensions.gh_management.github_planner.storage.os.unlink", side_effect=bad_unlink):
        with pytest.raises(OSError, match="replace failed"):
...
def test_read_issue_frontmatter_yaml_error_returns_none(workspace):
    ...
    with patch("extensions.gh_management.github_planner.storage.yaml.safe_load", side_effect=yaml.YAMLError("bad")):
