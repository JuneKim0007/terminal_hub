...
def test_check_auth_authenticated(workspace):
    ...
    with patch("extensions.gh_management.github_planner.get_workspace_root", return_value=workspace), \
         patch("extensions.gh_management.github_planner.resolve_token", return_value=("mytoken", TokenSource.ENV)):
...
def test_verify_auth_success(workspace):
    with patch("extensions.gh_management.github_planner.get_workspace_root", return_value=workspace), \
         patch("extensions.gh_management.github_planner.verify_gh_cli_auth", return_value=(True, "Verified.")):
        server = create_server()
...
def test_verify_auth_failure(workspace):
    with patch("extensions.gh_management.github_planner.get_workspace_root", return_value=workspace), \
         patch("extensions.gh_management.github_planner.verify_gh_cli_auth", return_value=(False, "Run: gh auth login")):
        server = create_server()
        result = call(server, "verify_auth", {})
    assert result["authenticated"] is False
    assert "options" in result
...
def test_update_project_description_write_failure(workspace):
    with patch("extensions.gh_management.github_planner.get_workspace_root", return_value=workspace), \
         patch("extensions.gh_management.github_planner.write_doc_file", side_effect=OSError("disk full")):
        server = create_server()
